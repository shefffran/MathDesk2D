...
public class Right_Point : MonoBehaviour
{
    public GameObject[] weight_prefabs;
    public GameObject[] weight_prefabs_start;
    public GameObject left_point;
    public GameObject board;
    public test_left_pos test_Left_Point;
    public test_left_bottom test_Left_Bottom;
    public int x = 0;
        ...
        if (other.gameObject.tag == "Weight")
        {
            if (x < 3)
            {
                ...
                if (x == 0)
                {
                    ...
                    //Instantiate(weight_prefabs_start[weight_index], start_pos_prefab[weight_index].transform.position, Quaternion.identity);
                ...
                if (x == 1)
                {
                    ...
                    weight_prefabs[weight_index].GetComponent<DistanceJoint2D>().connectedBody = test_Left_Point.weight_pos.gameObject.GetComponent<Rigidbody2D>();
                    //Instantiate(weight_prefabs_start[weight_index], start_pos_prefab[weight_index].transform.position, Quaternion.identity);
                ...
                {
                    weight_prefabs[weight_index].GetComponent<DistanceJoint2D>().connectedAnchor = new Vector2(0f, 0f);
                    weight_prefabs[weight_index].GetComponent<DistanceJoint2D>().connectedBody = test_Left_Bottom.weight_pos_bottom.gameObject.GetComponent<Rigidbody2D>();
                    //Instantiate(weight_prefabs_start[weight_index], start_pos_prefab[weight_index].transform.position, Quaternion.identity);
